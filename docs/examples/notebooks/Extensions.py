...
#
# The purpose of this notebook is to show the MODFLOW-API interface objects and introduce the user to
# the data types and how to interact with the objects.
#
# **Note**: This notebook shows how to run a model using the modflow-api at the end of the notebook.
# However, the majority of the notebook is an illustration of how to access and work with the data
# types that are returned to a user defined callback function.
...
#
# The modflow api allows users to initialize an object that can be used to interact with the model.
# This processes is done automatically with the `modflowapi.run_model` function call. We're going
# to initialize an object outside of that call as a demonstration of the interface data objects
...
#
# The `ApiSimulation` object is the top level container for the modflowapi interface classes.
# This container holds methods and other objects that allow the user to access boundary condition
# pointer data without assembling the specific memory addresses of the modflow data.
#
...
# and contains simulation level packages such as `sln`, `tdis`, `ats`, and `exchanges`
...
# ## The `ApiModel` object
#
# `ApiModel` objects are accessed from the `ApiSimulation` object and are a container for packages.
# These objects allow the user to view which packages are available and access those packages.
#
...
#
# Each package is contained in `ApiPackage` container. There are three types depending on the input data.
# We'll access and take a look at each of the types of `ApiPackage` containers.
...
# ### `ListPackage` objects
#
# `ListPackage` objects are the primary object type of stress period data. The exception to this rule
# is the advanced packages which will be discussed later.
#
...
#
# There are multiple ways to update values for `ListPackage` based data. The `.values` and `.dataframe`
# attributes can be used, or the object can be directly indexed if the user knows the underlying data.
...
#
# The `.stress_period_data` attribute returns a container class that interacts with the internal modflow
# pointers. The data can be adjusted by interacting with `.stress_period_data` in the same fashion as
# changing data in a numpy recarray.
...
# In list packages the user can add and remove specific boundary conditions. Note: if a user adds a
# boundary condition, such as another well during a stress period, the total number of wells cannot
...
# For the current stress period there are two active wells `nbound=2`, but there can be up to ten
# `maxbound=10`.
...
# do not contain any sort of stress period data. These packages are used primarily to define model
# connectivity, initial conditions, and hydraulic parameters of the basin.
...
#
# Two methods are available for accessing and updating data in `ArrayPackage` objects. `get_array()`
# and `set_array()` methods can be used to get and set data. Arrays can also be accessed as attributes
# on the object.
...
#
# Advanced variables in this context are variables that would not normally need to be accessed by the user,
# and in many cases changes to these variables would cause the Modflow simulation to do unexpected things.
...
# The user can access and change these values, _at their own risk_, using the `.get_advanced_var()` and
# `.set_advanced_var()` methods. Data is returned to the user in the internal modflowapi structure.

wel.get_advanced_var("ibcnum")

# ### Advanced Packages
#
# Certain packages only support accessing data through the `.get_advanced_var()` and `.set_advanced_var()`
# methods. These packages, are sometimes referred to as "advanced packages" and include: BUY, CSUB, GNC,
# HFB, MAW, MVR, SFR, and UZF.
...
#
# To run a simulation using the built in modflowapi runner the user needs to create a function that
# will receive callbacks at different steps in the simulation run. For the remainder of this notebook,
# we'll show how to create a callback function and use it with the `modflowapi.run_simulation()` method.
...
#       make adjustments before the model begins solving. This callback only occurs once at the beginning of
#       the MODFLOW6 simulation
#    - `Callbacks.stress_period_start`: the stress_period_start callback sends simulation data for each
#       solution group to the user to make adjustments to stress packages at the beginning of each stress period.
#    - `Callbacks.stress_period_end`: the stress_period_end callback sends simulation data for each solution
#       group to the user at the end of each stress period. This can be useful for writing custom output and
#       coupling models.
#    - `Callbacks.timestep_start`: the timestep_start callback sends simulation data for each solution group to
#       the user to make adjustments to stress packages at the beginning of each timestep.
#    - `Callbacks.timestep_end`: the timestep_end callback sends simulation data for each solution group to the
#       user at the end of each timestep. This can be useful for writing custom output and coupling models.
